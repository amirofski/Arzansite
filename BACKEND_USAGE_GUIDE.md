...
- POST `/wizard/save-session`
  ...
  - Note: If not yet deployed, frontend temporarily falls back to localStorage
...
- GET `/storage/:bucket` (list by bucket, e.g., `uploads`)
- GET `/storage/:bucket/:fileId/url` (signed URL)
- DELETE `/storage/:bucket/:fileId`
- Purpose: Upload/fetch/list/delete project files in storage
